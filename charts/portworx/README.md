...
1. Deploy StorageCluster CRD. Helm does not handle CRD upgrade, let's manually deploy it.
...
5. Wait for migration to complete Describe the StorageCluster to see event `Migration completed successfully`. If migration fails, there is corresponding event about the failure.
...
To use an external KVDB, you can provide the `etcdEndPoint`If the etcd cluster being used is a secured etcd (SSL/TLS) then please follow instructions to create a kubernetes secret with the certs. https://docs.portworx.com/scheduler/kubernetes/etcd-certs-using-secrets.html#create-kubernetes-secret
...
| `pxOperatorImageVersion` | Version of the PX operator image | 26.3.1 |
...
To uninstall/delete the `my-release` deployment: The chart would follow the process as outlined here. (https://docs.portworx.com/scheduler/kubernetes/install.html#uninstall)
